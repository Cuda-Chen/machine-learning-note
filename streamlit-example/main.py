...
# spending a few lines to describe out dataset
...
model.add(tf.keras.layers.Conv2D(32, kernel_size=(3, 3), activation=act1, input_shape=(32, 32, 3)))
...
act2 = st.selectbox('Activation function for Dense layer: ', ['relu', 'tanh', 'softmax'], key=0)
model.add(tf.keras.layers.Dense(1024, activation=act2))

act3 = st.selectbox('Activation function for Dense layer: ', ['relu', 'tanh', 'softmax'], key=1)
...
if st.checkbox('Fit model'):
    ...
    shuffle=True,
    epochs=epochs,
    validation_data=(x_test[0:1000] / 255.0, tf.keras.utils.to_categorical(y_test[0:1000]))
    )
...
preditions = model.predict(x_test / 255.0)
...
def plot_pred(i, preditions_array, true_label, img):
    preditions_array, true_label, img = preditions_array[i], true_label[i:i + 1], img[i]
    plt.grid(False)
...
def plot_bar(i, preditions_array, true_label):
    preditions_array, true_label = preditions_array[i], true_label[i]
    plt.grid(False)
    plt.yticks([])
    plt.xticks(np.arange(10), class_names, rotation=40)

    thisplot = plt.bar(range(10), preditions_array, color='gray')
    plt.ylim([0, 1])
...
if st.checkbox('Show random prediction results'):
    ...
    plt.subplot(1, 2, 1)
    plot_pred(num2, preditions, y_test, x_test)
    plt.subplot(1, 2, 2)
    plot_bar(num2, preditions, y_test)
    st.pyplot()
